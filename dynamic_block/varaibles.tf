...
variable "ingress_ports" {
  default = [ # list of map -> for this it works, for each.key and each.value for the for_each
    {
      from_port = 22 # i want to open for specific ports, so using dynamic block, as its ingress block
      to_port   = 22
    },
    {
      from_port = 80 # i want to open for specific ports, so using dynamic block, as its ingress block
      to_port   = 80
    },
    {
      from_port = 8080 # i want to open for specific ports, so using dynamic block, as its ingress block
      to_port   = 8080
    }
